...
function SkillSetsTextFormatter(props) {
    ...
    let matchingSkillSets = showMatchingsSkillSets.length > 0
    ? skillSets.filter(skillSet => showMatchingsSkillSets.some(matchingSet => matchingSet.id === skillSet.id))
    : skillSets;
    ...
    if (highlightMatchingsSkillSetIDs.length > 0) {
        return (
            <>
                {matchingSkillSets && matchingSkillSets.map(function(datum, i){
                    // Check if highlightMatchingsSkillSetIDs contains the current datum.id
                    const isHighlighted = highlightMatchingsSkillSetIDs.includes(datum.id);

                    // Determine the CSS class based on the condition
                    const cssClass = isHighlighted ? "tag is-success" : "tag is-success is-light";

                    return <span className={cssClass + " mr-2 mb-2"}>{datum.subCategory}</span>;
                })}
            </>
        );
    }
