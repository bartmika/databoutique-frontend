...
import { faPencil, faCircleCheck, faClose, faCheck, faCircleExclamation, faArrowRight } from '@fortawesome/free-solid-svg-icons';import { useRecoilState } from 'recoil';

import { topAlertMessageState, topAlertStatusState  } from "../../../AppState";
import { getAssociateAwayLogDetailAPI, putAssociateAwayLogUpdateAPI } from "../../../API/AssociateAwayLog";
...
function AssociateAwayLogUpdateModal({ currentUser, showUpdateModalForAssociateAwayLogID, setShowUpdateModalForAssociateAwayLogID, onSuccessUpdateCallbackFunc }) {
    ////
    //// Global state.
    ////
    ...
    useEffect(() => {
        ...
        if (mounted) {
            ...
            if (showUpdateModalForAssociateAwayLogID !== undefined && showUpdateModalForAssociateAwayLogID !== null && showUpdateModalForAssociateAwayLogID !== "") {
                setFetching(true);
    ...
    return (
        <div class={`modal ${showUpdateModalForAssociateAwayLogID ? 'is-active' : ''}`}>
            <div class="modal-background"></div>
            <div class="modal-card">
                <header class="modal-card-head">
                    <p class="modal-card-title"><FontAwesomeIcon className="mdi" icon={faPencil} />&nbsp;Edit</p>
                    <button class="delete" aria-label="close" onClick={onCloseModal}></button>
                </header>
                        ...
                        <div class="content">
                            ...
                            <>
                                ...
                                <FormRadioField
                                    ...
                                    opt2Value={2}
                                    opt2Label="No"
                                    onChange={(e)=>setUntilFurtherNotice(parseInt(e.target.value))}
                                />
                ...
                {isFetching ?
                    ...
                    <footer class="modal-card-foot">
                        <button class="button" onClick={onCloseModal}>
                            <FontAwesomeIcon className="mdi" icon={faClose} />&nbsp;Cancel
                        </button>
                        <button class="button is-success" onClick={onSaveClick}>
                            <FontAwesomeIcon className="mdi" icon={faCheck} />&nbsp;Confirm and Save
                        </button>
                    </footer>
                }
            </div>
