...
import { faArrowCircleRight, faHome, faBuildingUser, faHomeUser, faUserGear, faArrowRight, faArrowLeft, faSearch, faTasks, faTachometer, faPlus, faTimesCircle, faCheckCircle, faUserCircle, faGauge, faPencil, faUsers, faIdCard, faAddressBook, faContactCard, faChartPie, faBuilding, faClose } from '@fortawesome/free-solid-svg-icons'
...
import { RESIDENTIAL_USER_TYPE_OF_ID, COMMERCIAL_USER_TYPE_OF_ID  } from "../../../../Constants/App";
import { addUserState, ADD_USER_STATE_DEFAULT, currentUserState } from "../../../../AppState";


function AdminUserAddStep2() {
    ////
    //// Global state.
    ////
    ...
    const onSelectType = (to) => {
        // Let's create a clone of our user.
        let modifiedAddUser = { ...addUser };

        // Set the type.
        modifiedAddUser.type = to;

        // Attach the users country based on current user's country if
        // the logged in user has a country, if not then default to `Canada`,
        // Why are we doing this? Because the phone number formatter requires a
        // country while we ask about country in the wizard before.
        if (currentUser.country !== undefined && currentUser.country !== null && currentUser.country !== "") {
            modifiedAddUser.country = currentUser.country;
        } else {
            modifiedAddUser.country = "Canada";
        }

        // Save to persistent storage our new user.
                        ...
                        <div className={`modal ${showCancelWarning ? 'is-active' : ''}`}>
                            ...
                            <div className="modal-card">
                                ...
                                <footer className="modal-card-foot">
                                    <Link className="button is-medium is-success" to={`/admin/users/add/step-1-search`}>Yes</Link>
                                    <button className="button is-medium" onClick={(e)=>setShowCancelWarning(false)}>No</button>
                                </footer>
                            </div>
                        </div>

                        <p className="title is-4"><FontAwesomeIcon className="fas" icon={faUserGear} />&nbsp;Select User Type:</p>

                        <p className="pb-4 has-text-grey">Please select the type of user this is.</p>
                                ...
                                <div className="columns">
                                        ...
                                        <div className="card">
                                            <div className="card-image has-background-info">
                                                <div className="has-text-centered" style={{padding:"60px"}}>
                                                    <FontAwesomeIcon className="fas" icon={faHome} style={{ color: 'white', fontSize: '9rem' }} />
                                                </div>
                                            </div>
                                            <div className="card-content">
                                                <div className="media">
                                                  ...
                                                  <div className="media-content">
                                                    <p className="title is-4"><FontAwesomeIcon className="fas" icon={faHomeUser} />&nbsp;Residential User</p>
                                                  </div>
                                                </div>

                                                <div className="content">
                                                    Add a Residential User.
                                                    <br />
                                                </div>
                                            </div>
                                            <footer className="card-footer">
                                                <button onClick={(e,s)=>onSelectType(RESIDENTIAL_USER_TYPE_OF_ID)} type="button" className="card-footer-item button is-primary is-large">
                                    ...
                                    <div className="column">
                                        <div className="card">
                                            <div className="card-image has-background-info">
                                                <div className="has-text-centered" style={{padding:"60px"}}>
                                                    <FontAwesomeIcon className="fas" icon={faBuilding} style={{ color: 'white', fontSize: '9rem' }} />
                                                </div>
                                            </div>
                                            <div className="card-content">
                                                <div className="media">
                                                  ...
                                                  <div className="media-content">
                                                    <p className="title is-4"><FontAwesomeIcon className="fas" icon={faBuildingUser} />&nbsp;Business User</p>
                                                  </div>
                                                </div>
                                            ...
                                            <footer className="card-footer">
                                                <button onClick={(e,s)=>onSelectType(COMMERCIAL_USER_TYPE_OF_ID)} type="button" className="card-footer-item button is-primary is-large">
                                                    Pick&nbsp;<FontAwesomeIcon className="fas" icon={faArrowCircleRight} />
                                                </button>
                                            </footer>
                                        </div>
...
export default AdminUserAddStep2;
