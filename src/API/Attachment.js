...
import {
    DATABOUTIQUE_ATTACHMENTS_API_ENDPOINT,
    DATABOUTIQUE_ATTACHMENT_API_ENDPOINT,
} from "../Constants/API";
...
export function getAttachmentListAPI(filtersMap=new Map(), onSuccessCallback, onErrorCallback, onDoneCallback, onUnauthorizedCallback) {
    ...
    let aURL = DATABOUTIQUE_ATTACHMENTS_API_ENDPOINT;
    filtersMap.forEach(
        (value, key) => {
...
export function postAttachmentCreateAPI(formdata, onSuccessCallback, onErrorCallback, onDoneCallback, onUnauthorizedCallback) {
    ...
    axios.post(DATABOUTIQUE_ATTACHMENTS_API_ENDPOINT, formdata, {
        headers: {
...
export function getAttachmentDetailAPI(attachmentID, onSuccessCallback, onErrorCallback, onDoneCallback, onUnauthorizedCallback) {
    const axios = getCustomAxios(onUnauthorizedCallback);
    axios.get(DATABOUTIQUE_ATTACHMENT_API_ENDPOINT.replace("{id}", attachmentID)).then((successResponse) => {
        const responseData = successResponse.data;
...
export function putAttachmentUpdateAPI(id, data, onSuccessCallback, onErrorCallback, onDoneCallback, onUnauthorizedCallback) {
    ...
    axios.put(DATABOUTIQUE_ATTACHMENT_API_ENDPOINT.replace("{id}", id), data, {
        headers: {
          'Content-Type': 'multipart/form-data',
          'Accept': 'application/json'
        }
...
export function deleteAttachmentAPI(id, onSuccessCallback, onErrorCallback, onDoneCallback, onUnauthorizedCallback) {
    const axios = getCustomAxios(onUnauthorizedCallback);
    axios.delete(DATABOUTIQUE_ATTACHMENT_API_ENDPOINT.replace("{id}", id)).then((successResponse) => {
        const responseData = successResponse.data;
